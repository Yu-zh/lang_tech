...
def index_one_file(dir,file_name):
    ...
    iter_in_text = re.finditer(r'\p{L}+', text.lower())
...
def tf_idf_matrix():
    ...
    word_count_book = len_word.sum(1)
    book_count_word = pd.DataFrame(len_word != 0).sum(0)
...
if __name__ == "__main__":
    ...
    # test part
    print("gjord:",word_index["gjord"]["bannlyst.txt"])
    print("uppklarnande:",word_index["uppklarnande"]["bannlyst.txt"])
    print("stjärnor:",word_index["stjärnor"]["bannlyst.txt"])
    print()
    print("samlar:",word_index["samlar"])
    print("ände",word_index["ände"])
